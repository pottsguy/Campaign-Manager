...
def generator_view(request):
    ...
    "generate a new medieval fantasy quest hook in 1 short sentence"
    ], stdout=subprocess.PIPE, encoding="utf-8").stdout.strip()

    villager = subprocess.run(["ollama", "run", "mistral",
    "generate the name, job and quirk of a new medieval fantasy commoner in 1 short sentence"
    ], stdout=subprocess.PIPE, encoding="utf-8").stdout.strip()

    adventurer = subprocess.run(["ollama", "run", "mistral",
    "generate the name, background and quirk of a new medieval fantasy adventurer in 1 short sentence"
    ], stdout=subprocess.PIPE, encoding="utf-8").stdout.strip()

    monster = subprocess.run(["ollama", "run", "mistral",
    "in one short sentence, generate the name, habitat and interesting ability/habit of a new medieval fantasy animal/monster"
    ], stdout=subprocess.PIPE, encoding="utf-8").stdout.strip()

    weapon = subprocess.run(["ollama", "run", "mistral",
    "generate the name, appearance and desire of a new medieval fantasy magical weapon in 1 short sentence"
    ], stdout=subprocess.PIPE, encoding="utf-8").stdout.strip()
